...
public class Sql2oTaskDao implements TaskDao { //implementing our interface
    ...
    public void add(Task task) {
        String sql = "INSERT INTO tasks (name, description, categoryId) VALUES (:name, :description, :categoryId)"; //raw sql
    ...
    @Override
    public void update(int id, String newName, String newDescription, int newCategoryId){
        String sql = "UPDATE tasks SET (name, description, categoryId) = (:name, :description, :categoryId) WHERE id=:id";   //raw sql
        try(Connection con = sql2o.open()){
            con.createQuery(sql)
